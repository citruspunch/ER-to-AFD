...
//Quitamos posiciones que se hayan agregado repetidas
  ...
  public static void main(String[] args) throws Exception{
    ...
    int[] estadosFinalesArray = new int[estadosFinales.size()];
    for (int i = 0; i < estadosFinales.size(); i++) {
        estadosFinalesArray[i] = estadosFinales.get(i);
    }
    ...
    if (opcion == 1){
      ...
      int[][] afd = transicionesAFD(hashnuevo, estado);
      int cantEstados = afd[0].length;
      er.toGLD(true, cantEstados, afd, estadosFinalesArray, alfabeto);
    } else if (opcion == 3){
      int[][] afd = transicionesAFD(hashnuevo, estado);
      int cantEstados = afd[0].length;
      er.minimizarAFD(true, cantEstados, afd, estadosFinalesArray, alfabeto);
    } else if (opcion == 4){
      int[][] afd = transicionesAFD(hashnuevo, estado);
      while(true){
        ...
        boolean aceptado = er.parsingAFD(cuerda, afd, estadosFinalesArray);
        if (aceptado){
